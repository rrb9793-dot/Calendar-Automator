...
async function fetchUserPreferences() {
    ...
    if (email && email.includes('@')) {
        ...
        try {
            ...
            console.log('No saved preferences found (New User).');
...
if (emailInput) {
    // 1. Trigger on "Blur" (Clicking away)
    emailInput.addEventListener('blur', fetchUserPreferences);

    // 2. Trigger on "Enter" Key
    emailInput.addEventListener('keydown', (e) => {
        if (e.key === 'Enter') {
            e.preventDefault(); // Stop standard form submit behavior
            fetchUserPreferences();
            emailInput.blur(); // Remove focus to signal "done"
        }
    });
} // --- CONFIGURATION ---
const MAJORS = [
