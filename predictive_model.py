...
# STRICT COLUMN DEFINITION (Matches your OHE drop_first=True logic)
MODEL_COLUMNS = [
    'work_sessions',
...
if os.path.exists(CSV_PATH):
    try:
        ...
        new_column_names = {
            ...
            'Second concentration? (if none, select N/A)': 'second_concentration', 'Minor? (if none select N/A)': 'minor',
            'What class was the assignment for (Please write as said in BrightSpace)': 'class_name',
            'What field of study was the assignment in?': 'field_of_study', 'What type of assignment was it?': 'assignment_type',
            'Approximately how long did it take (in hours)': 'time_spent_hours', 'What was the extent of your reliance on external resources? ': 'external_resources',
            'Where did you primarily work on the assignment?': 'work_location', 'Did you work in a group?': 'worked_in_group',
        ...
        for col in ['major', 'second_concentration', 'minor', 'field_of_study']:
            if col in survey_df.columns: survey_df[f'{col}_category'] = survey_df[col].map(category_mapping)
        ...
        work_location_mapping = {'At home/private setting': 'home', 'School/library': 'school', 'Other public setting (cafe, etc.)': 'public'}
...
def predict_assignment_time(user_profile, assignment_details):
    if model is None:
        print("⚠️ Model is None, returning default 2.0")
        return 2.0

    FRONTEND_MAP = {
        "Business": "business", "Tech & Data Science": "tech_data", "Engineering": "engineering",
        "Math": "math", "Natural Sciences": "natural_sciences", "Social Sciences": "social_sciences_law",
        "Arts & Humanities": "arts_humanities", "Health & Education": "health_education",
        "Problem Set": "p_set", "Coding Assignment": "coding", "Research Paper": "research_paper",
        "Creative Writing/Essay": "essay", "Presentation": "presentation", "Modeling": "modeling",
        "Discussion Post": "discussion", "Readings": "readings", "Case Study": "case_study",
        "Textbook / class materials": "class_materials", "Google/internet": "google", "AI / Chatgpt": "ai",
        "At home/private setting": "home", "School/library": "school", 
        "Other public setting (cafe, etc.)": "public", "Yes": "Yes", "No": "No"
    }

    input_vector = {col: 0 for col in MODEL_COLUMNS}
    active_features = [] # [DEBUG] Track what features are actually turning on
    ...
    def set_feature(prefix, raw_value):
        ...
        if mapped_val:
            ...
            # else: Reference category or missing column (e.g. 2026, Humanities)
        else:
            if raw_value and raw_value != 'N/A':
                print(f"⚠️ MAPPING FAIL: '{raw_value}' not found in FRONTEND_MAP")

    try:
        ws = int(assignment_details.get('work_sessions', 1))
        input_vector['work_sessions'] = ws
        active_features.append(f"sessions={ws}")
    except:
        input_vector['work_sessions'] = 1

    year_val = user_profile.get('year') 
    if f"year_{year_val}" in input_vector:
        input_vector[f"year_{year_val}"] = 1
        active_features.append(f"year_{year_val}")
    else:
        # This is expected for 2026
        pass 
    ...
    print(f"🔍 INPUT FEATURES: {active_features}")
